...
import { archiveAgentThread, createAgentThread, getRuntimeStatus, interruptAgentTurn, listAgentModels, listAgentThreads, loadProviderConfig, onAppServerNotification, onAppServerRequest, readAgentThread, renameAgentThread, respondToApproval, resumeAgentThread, saveProviderConfig, startAgentTurn, startRuntime, stopRuntime, type AppServerMessage, type RuntimeSnapshot } from "./runtime-client";
...
function Workspace({ engine, runtime, configuredModel, cwd, blocks, onCwd, activeSession, onSession, onSessions, onActiveSession, onRuntime }: WorkspaceProps) {
  ...
  useEffect(() => {
    ...
    void onAppServerNotification((message) => { const entry = normalizeRuntimeEvent(message); if (entry) setTimeline((current) => mergeTimelineEntry(current, entry).slice(-200)); if (message.method === "turn/started") { const id = nestedId(message, "turn"); if (id) setTurnId(id); } if (message.method?.includes("diff")) setDiff(String(message.params?.diff ?? "")); if (message.method === "turn/completed" || message.method === "turn/end") { setBusy(false); setTurnId(null); } }).then((cleanup) => cleanups.push(cleanup));
...
function Inspector({ engine, runtime, cwd, stats, diff, blocks, onProject }: { engine: RuntimeEngine; runtime: RuntimeSnapshot; cwd: string; stats: { commands: number; files: number }; diff: string; blocks: BlockPreference[]; onProject(): Promise<void> }) {
  ...
  return <aside className="inspector-pane"><header><b>任务上下文</b><span>{visible.length} 块积木</span></header>{visible.map((block) => {
    ...
    if (block.id === "diff") return <section key={block.id} className="changes"><small>代码变更</small>{diff ? <pre>{diff}</pre> : <p>任务产生的 Diff 会显示在这里，提交前可逐项审阅。</p>}</section>;
    if (block.id === "capabilities") return <section key={block.id}><small>可用能力</small><div className="capability-list">{Object.entries(runtimeCatalog[engine].capabilities).filter(([name, value]) => value && capabilityEnabled(name, enabledBlockIds)).map(([name]) => <span key={name}>✓ {capabilityName(name)}</span>)}{runtimeBlocks.map((item) => <span key={item.id}>✓ {item.name}</span>)}</div></section>;
    return null;
  })}</aside>;
}
