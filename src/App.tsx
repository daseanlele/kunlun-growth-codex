...
function Workspace({ engine, runtime, configuredModel, cwd, blocks, onCwd, activeSession, onSession, onSessions, onActiveSession, onRuntime }: WorkspaceProps) {
  ...
  }, [engine, runtime.status, configuredModel, cwd]);
  useEffect(() => {
  ...
  async function send() {
    ...
    try {
      ...
      const selectedSkill = availableSkills.find((skill) => skill.path === selectedSkillPath); const started = await startAgentTurn(currentThread, cwd, text, selectedModel || configuredModel, effort, images, selectedSkill); setImages([]); const startedTurn = nestedId(started, "turn"); if (startedTurn) setTurnId(startedTurn);
...
interface ProviderProps { runtime: RuntimeSnapshot; baseUrl: string; model: string; protocol: ProviderProtocol; apiKey: string; saved: boolean; onRuntime(value: RuntimeSnapshot): void; onBaseUrl(value: string): void; onModel(value: string): void; onProtocol(value: ProviderProtocol): void; onApiKey(value: string): void; onSave(): void }
function ProviderSettings(props: ProviderProps) {
  const [loginUrl, setLoginUrl] = useState(""); const [loginError, setLoginError] = useState(""); const active = matchProvider(props.baseUrl);
  function selectProvider(provider: ProviderPreset) { props.onProtocol(provider.protocol); props.onBaseUrl(provider.baseUrl); props.onModel(provider.suggestedModels[0] ?? ""); setLoginUrl(""); }
  ...
  return <section className="settings-page provider-page"><header><small>设置</small><h1>账号与模型</h1><p>直接连接智能体账号，或从全球模型服务中选择供应商；任务界面始终保持统一。</p></header>
    ...
    <div className="settings-section"><h2>当前配置 · {active.name}</h2><div className="form-grid"><label>协议<select value={props.protocol} onChange={(event) => props.onProtocol(event.target.value as ProviderProtocol)}><option value="openai">OpenAI Responses</option><option value="azure-openai">Azure OpenAI</option><option value="openai-compatible">OpenAI-compatible</option></select></label><label>模型 ID<input list="provider-models" value={props.model} onChange={(event) => props.onModel(event.target.value)} placeholder="填写或选择模型" /><datalist id="provider-models">{active.suggestedModels.map((item) => <option key={item} value={item} />)}</datalist></label><label className="full">API Base URL<input value={props.baseUrl} onChange={(event) => props.onBaseUrl(event.target.value)} /></label><label className="full">API Key<input type="password" value={props.apiKey} onChange={(event) => props.onApiKey(event.target.value)} placeholder="安全保存到系统凭据库，不写入项目配置" /></label></div><div className="settings-actions"><button className="primary" disabled={!props.baseUrl.startsWith("https://") || active.id === "anthropic"} onClick={props.onSave}>{active.id === "anthropic" ? "请使用 Claude 原生连接" : props.saved ? "已保存 ✓" : "保存并切换"}</button></div></div>
  </section>;
}
