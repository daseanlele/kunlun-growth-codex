...
export function App() {
  ...
  async function saveAndActivateProvider() {
    try {
      const savedConfig = await saveProviderConfig({ id: profileId, providerId: providerId || "custom", displayName: providerName || "自定义模型", protocol, adapter, baseUrl, model, authMethod: "api-key", credentialRef: null }, apiKey);
  ...
  function applyProvider(config: ProviderConfigPayload) { setProfileId(config.id); setBaseUrl(config.baseUrl); setModel(config.model); setProtocol(config.protocol); setAdapter(config.adapter); setProviderId(config.providerId); setProviderName(config.displayName); }
  ...
  return <main className="window-shell">
    ...
    <div className="app-frame">
      ...
      {view === "providers" && <ProviderSettings runtime={runtime} baseUrl={baseUrl} model={model} protocol={protocol} adapter={adapter} profileId={profileId} providerId={providerId} providerName={providerName} profiles={profiles} apiKey={apiKey} saved={saved} onRuntime={setRuntime} onBaseUrl={setBaseUrl} onModel={setModel} onProtocol={setProtocol} onAdapter={setAdapter} onProfileId={setProfileId} onProviderId={setProviderId} onProviderName={setProviderName} onActivate={(id) => void activateProfile(id)} onApiKey={setApiKey} onSave={() => void saveAndActivateProvider()} />}
...
interface ProviderProps { runtime: RuntimeSnapshot; baseUrl: string; model: string; protocol: ProviderProtocol; adapter: ProviderAdapter; profileId: string; providerId: string; providerName: string; profiles: ProviderConfigPayload[]; apiKey: string; saved: boolean; onRuntime(value: RuntimeSnapshot): void; onBaseUrl(value: string): void; onModel(value: string): void; onProtocol(value: ProviderProtocol): void; onAdapter(value: ProviderAdapter): void; onProfileId(value: string): void; onProviderId(value: string): void; onProviderName(value: string): void; onActivate(id: string): void; onApiKey(value: string): void; onSave(): void }
