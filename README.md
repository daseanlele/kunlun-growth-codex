...
Codex / OpenAI Responses 适配器使用 App Server，因此具备完整的代理工具、审批、Skills、MCP、Review 与协作能力。Claude 与 Chat Completions 适配器使用各自原生 API，不会伪装为 Responses 协议；当前已支持流式多轮对话、模型发现、停止、受限 `@文件` 上下文，以及列目录、读文件、读 Git Diff、受限全文搜索的工具循环。模型档案支持选择认证头名称（如 `Authorization`、`api-key` 或企业网关的 `X-API-Key`），密钥仍仅保存在系统凭据库。原生模型可请求创建或覆盖 UTF-8 文本文件；每一次写入都会在桌面端要求用户明确批准，路径严格限定在所选工作区，且不会删除或重命名文件。执行命令、网络访问、MCP 与子代理工具循环仍在接入中，未接入前不会绕过企业审批边界。
