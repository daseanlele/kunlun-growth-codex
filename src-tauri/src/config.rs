...
pub fn save(app: &AppHandle, mut config: ProviderConfig, api_key: Option<String>) -> Result<ProviderConfig, String> {
    ...
    if let Some(secret) = api_key.filter(|value| !value.trim().is_empty()) {
        ...
        entry.set_password(secret.trim()).map_err(|error| error.to_string())?;
